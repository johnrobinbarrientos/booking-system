...
@section('subcontent')
    ...
    <div class="intro-y col-span-12 w-2/3">
        <form action="" method="POST" enctype="multipart/form-data" class="flex">
            ...
            <a href="" class="btn btn-outline-primary w-auto">Download Sample Import</a>
        </form>
    </div>-->
    ...
    <div class="grid grid-cols-12 gap-6 mt-5">
        <div class="intro-y col-span-12 flex flex-wrap sm:flex-nowrap items-center mt-2">
            <a href="{{ route('pumpMake.create') }}" class="btn btn-primary shadow-md mr-2">Add New Manufacturer</a>
            ...
            <div class="w-full xl:w-auto flex items-center mt-3 xl:mt-0">
                ...
                <div class="w-56 relative text-slate-500">
                    <form action="{{ route('pumpMake.index') }}" method="GET">
                        @csrf
        ...
        <div class="intro-y col-span-12 overflow-auto lg:overflow-visible">
            ...
            <table class="table mt-2">
                <thead>
                    <tr>
                        <th class="text-center">Pump Manufacturer</th>
                        <th class="text-center">ACTIONS</th>
                    </tr>
                </thead>
                <tbody>
                    @forelse ($pumpMakes as $pumpMake)
                        <tr class="text-center">
                            <td class="text-center">{{ $pumpMake->make }}</td>
                            <td class="table-report__action w-56">
                              <a href="{{route('pumpMake.edit', $pumpMake->id)}}" class="btn btn-warning mr-1 py-1 px-2">
                                   <i data-lucide="check-square" class="w-4 h-4 mr-1"></i> Edit</a>
                                  
                              <a class="btn btn-danger py-1 px-2" data-tw-toggle="modal"
                                   data-tw-target="#delete-confirmation-modal_{{ $pumpMake->id }}"
                                   data-action="{{ route('pumpMake.destroy', $pumpMake->id) }}">
                                   <i data-lucide="trash-2" class="w-4 h-4 mr-1"></i>
                                   Delete</a>

                                   <!-- BEGIN: Delete Confirmation Modal -->
                                <div id="delete-confirmation-modal_{{ $pumpMake->id }}" class="modal" tabindex="-1"
                                   aria-hidden="true">
                                   <div class="modal-dialog">
                                       <div class="modal-content">
                                           <form method="POST" action="{{ route('pumpMake.destroy', $pumpMake->id) }}">
                                               @csrf
                                               @method('DELETE')
